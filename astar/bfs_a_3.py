...
# Uses a first in first out queue in stead of a priority queue

# The main bfs algorithm
def run_bfs(board, start_node, end_node):
    ...
    priority_queue = []  # The opened nodes, FIFO queue
    ...
    while priority_queue:  # Start searching if the opened nodes is not empty
        current_node = priority_queue[0]   # Chose a current node, based on FIFO in the opened queue
        ...
        for adj_node in adj_nodes:  # If the adjacent node is already closed, then don't calculate it again
            ...
            if adj_node.walkable and adj_node not in came_from:  # Update the node if it is walkable and not already update
                ...
                priority_queue.append(adj_node)  # Add the node to the open queue
...
if __name__ == "__main__":  # Create board and run bfs
    board_data = read_board(board_id="2-4")  # run_board returns board, start, end as a tuple
    run_bfs(*board_data)  # tuple unpacking, spread tuple as arguments
